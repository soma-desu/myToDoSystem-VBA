...
> **次の就業日**は「全休日を除いた翌営業日」を算出する純粋関数で求める: `Function GetNextWorkDay(today As Date, holidays As Collection) As Date`
> 
...
```
SheetPresenterBasicToDoValidatorToDoServiceExcelToDoRepositoryEventDispatcherWorksheet EventExcel UserSheetPresenterBasicToDoValidatorToDoServiceExcelToDoRepositoryEventDispatcherWorksheet EventExcel Useralt[Valid][Invalid]セル値変更 (rewrite)Dispatch(rawEvent)FindAll() [IO:入力]Collection of BasicToDoEditBasicToDo(id, inputDTO)Validate(inputDTO) [純粋]ValidationResultServiceResult(success, updatedEntity)Save(updatedEntity) [IO:出力]RenderToDoList(sheet, rows) [IO:出力]ServiceResult(failure, errors)エラーメッセージ表示
```
